{...}: {
  flake.modules.homeManager.shells = {
    ...
    options.limonene.defaultShell = lib.mkOption {
      type = lib.types.package;
      default = pkgs.fish;
      description = "The user's default interactive shell package.";
    };
    ...
    config = {
      ...
      programs.nushell = {
        ...
        extraEnv = ''
          $env.PATH = (
            $env.PATH
            | prepend [${lib.concatMapStringsSep ", " (p: "\"${p}\"") config.home.sessionPath}]
            | uniq
          )
        '';
      };
