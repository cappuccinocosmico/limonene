...
-- /home/nicole/.config/nvim/lua/plugins/highlighting.lua:94: '<eof>' expected near 'end'
...
local function test_oklch_to_hex()
  ...
  for _, test in ipairs(tests) do
    ...
    assert(
      result_hex == test.expected_hex,
      string.format("Expected %s but got %s for %s", test.expected_hex, result_hex, test.oklch)
    )
  end
